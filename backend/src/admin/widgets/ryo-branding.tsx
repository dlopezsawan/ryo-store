...
function RyoBranding() {
  useEffect(() => {
    ...
    function injectStyles() {
      ...
      style.textContent = `
        .ryo-logo-replaced {
          ...
          background-image: url("${BICHITO_URL}") !important;
    ...
    function applyBranding() {
      ...
      if (fav && !fav.href.includes("bichito")) {
        fav.href = BICHITO_URL
      ...
      // Login page: replace the large Medusa icon
      // The Medusa logo on login is inside a rounded dark box, typically a div > svg
      document.querySelectorAll("form").forEach((form) => {
        const container = form.closest("div")
        if (!container) return
        const parent = container.parentElement
        if (!parent) return
        // Find all SVGs before the form (the logo area)
        parent.querySelectorAll("svg").forEach((svg) => {
          if (svg.closest("form")) return
          const rect = svg.getBoundingClientRect()
          if (rect.width > 20 && rect.height > 20 && rect.width < 200) {
            const wrapper = svg.parentElement
            if (wrapper && !wrapper.querySelector(".ryo-login-logo")) {
              svg.style.display = "none"
              const img = document.createElement("img")
              img.src = BICHITO_URL
              img.className = "ryo-login-logo"
              img.style.width = `${Math.max(rect.width, 40)}px`
              img.style.height = `${Math.max(rect.height, 40)}px`
      ...
      // Sidebar: replace the letter avatar (the "R" or "M" box)
      // These are small square divs with a single character, inside links
      document.querySelectorAll("aside a, aside button").forEach((el) => {
        el.querySelectorAll("div").forEach((div) => {
          const text = div.textContent?.trim() || ""
          const rect = div.getBoundingClientRect()
          // Target small square boxes (avatars) with single characters
          if (
            text.length === 1 &&
            rect.width >= 20 && rect.width <= 40 &&
            rect.height >= 20 && rect.height <= 40 &&
            !div.classList.contains("ryo-logo-replaced")
          ) {
            div.classList.add("ryo-logo-replaced")
          }
        })
      })

      // Also target the store switcher dropdown avatar
      document.querySelectorAll("[role='dialog'] div, [data-radix-popper-content-wrapper] div").forEach((div) => {
        const text = div.textContent?.trim() || ""
        const rect = div.getBoundingClientRect()
        if (
          text.length === 1 &&
      ...
      document.querySelectorAll("h1, h2, h3, p, span").forEach((el) => {
        ...
        if (t.includes("Welcome to Medusa")) el.textContent = t.replace("Welcome to Medusa", "Bienvenido a Enrola")
        if (t.includes("Bienvenido a Medusa")) el.textContent = t.replace("Bienvenido a Medusa", "Bienvenido a Enrola")
        if (t === "Medusa Store") el.textContent = "Enrola Shop"
        if (t === "Medusa") el.textContent = "Enrola Shop"
      })

      // Document title
      if (document.title.includes("Medusa")) {
        document.title = document.title.replace(/Medusa/g, "Enrola Shop")
      }
    }
