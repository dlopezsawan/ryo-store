...
function applyDashboardBranding() {
  ...
  // Replace all single-letter avatar boxes in the sidebar
  document.querySelectorAll("aside div, nav div").forEach((div) => {
    if (div.classList.contains("ryo-avatar-replaced")) return
    ...
    if (w >= 20 && w <= 44 && h >= 20 && h <= 44) {
      const bg = window.getComputedStyle(el).backgroundColor
      if (bg && bg !== "rgba(0, 0, 0, 0)" && bg !== "transparent") {
        el.classList.add("ryo-avatar-replaced")
      }
    }
  })

  // Popover/dropdown avatars
  document.querySelectorAll("[data-radix-popper-content-wrapper] div, [role='dialog'] div").forEach((div) => {
    if (div.classList.contains("ryo-avatar-replaced")) return
    if (div.children.length > 1) return
    const el = div as HTMLElement
    const text = el.innerText?.trim() || ""
  ...
  document.querySelectorAll("aside span, aside p, nav span").forEach((el) => {
...
function DashboardBranding() {
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => requestAnimationFrame(applyDashboardBranding))
